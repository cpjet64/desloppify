...
Install it with `desloppify update-skill codex_loop95`. The native Codex target is `~/.codex/skills/desloppify-loop95/SKILL.md`, and the install also writes `~/.codex/skills/desloppify-loop95/agents/openai.yaml` so explicit `$desloppify-loop95` invocation gets a real starter prompt. Use `--scope project` only if you intentionally need the legacy repo-local compatibility install.
...
8. If Codex does not show the new loop behavior immediately after reinstall, restart Codex so it reloads the installed skill metadata.
