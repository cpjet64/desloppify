...
If you want Codex to treat `strict >= 95` as an explicit stop condition, install `desloppify update-skill codex_loop95` instead. That variant installs a standalone loop-specific skill, Codex metadata, a session-scoped Ralph loop hook under `~/.codex/hooks.json`, and enables `codex_hooks` so `$desloppify-loop95` both starts and enforces the loop.
