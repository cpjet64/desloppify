...
- `codex_loop95` is a standalone, loop-specific skill. It does not inherit the full generic Codex skill body.
- Once `$desloppify-loop95` is activated in a session, the Ralph loop hook enforces this order: scan, run subjective review for the current scan when below target, clear open review work, execute the living plan via `next`, consult `desloppify plan queue` when execution empties, follow its exact promotion guidance if backlog remains, then rescan when the current work chunk is exhausted. While review is pending or open, do not inspect backlog or continue code work. If the repo is genuinely blocked below target, end with `LOOP95_BLOCKED:` plus the exact command, exact error, and current strict score.
